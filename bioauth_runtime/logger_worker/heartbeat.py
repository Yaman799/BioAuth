"""Logger-only heartbeat publisher with atomic writes and lock hardening."""
...
def write_logger_heartbeat_payload(payload: Mapping[str, object] | None) -> bool:
    """Write logger heartbeat atomically; retry WinError 5 without stopping capture."""
    ...
    data.setdefault("worker_kind", "logger")
    ...
    data.update({
        ...
        "heartbeat_file_path": worker_heartbeat_path("logger"),
    })
    path = Path(worker_heartbeat_path("logger"))
...
def clean_stale_logger_temp_heartbeats() -> int:
    """Remove stale logger heartbeat temp files only."""
    path = Path(worker_heartbeat_path("logger"))
    path.parent.mkdir(parents=True, exist_ok=True)
    removed = 0
...
def _warn_permission_error(exc: PermissionError, path: str) -> None:
    ...
    LOGGER.warning("Logger heartbeat write was blocked by the OS; will retry: %s (%s)", path, exc)
...
def _safe_unlink(path: Path) -> bool:
    try:
        ...
        LOGGER.debug("Could not remove logger heartbeat temp file %s", path, exc_info=True)
        return False
