...
def show_taskbar_notification(
    ...
    *,
    ...
    """Show a best-effort taskbar balloon without blocking BioAuth startup.

    ``bridge.shared`` imports this top-level module while the desktop app is
    starting.  Notifications are optional; if Windows notification delivery is
    unavailable, callers receive ``False`` and the UI can continue opening.
    """
    if os.name != "nt":
        return False
    ...
    if not ps:
        return False
    try:
        script = _build_balloon_script(str(title or "BioAuth"), str(message or ""), timeout_ms=int(timeout_ms), level=level)
...
__all__ = ["NotificationLevel", "show_taskbar_notification"]
